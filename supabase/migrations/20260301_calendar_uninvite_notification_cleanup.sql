-- Remove stale invite notifications when an invite is deleted (uninvite flow).
-- Keep existing invite/update/remove notification behavior.
...
begin
  ...
  if tg_op = 'DELETE' and row_invitee_id is not null and row_invitee_id <> row_inviter_id then
    delete from public.calendar_notifications n
    where n.user_id = row_invitee_id
      and n.type = 'plan_invite'
      and (
        coalesce(n.payload->>'invite_id', '') = coalesce(row_invite_id::text, '')
        or coalesce(n.payload->>'plan_id', '') = coalesce(row_plan_id, '')
      );
